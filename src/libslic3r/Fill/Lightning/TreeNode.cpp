...
void Node::convertToPolylines(Polylines &output, const coord_t line_overlap) const
{
    Polylines result;
    result.emplace_back();
    // Orca: the layers are filled in parallel, so a generator shared between them would be drawn
    // from in a different order every run and a model would not slice the same way twice. Each tree
    // seeds its own from where it is rooted; one constant seed would start them all on the same pick.
